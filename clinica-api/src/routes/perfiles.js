	...
	try {
		...
		if (!medico) {
			return res.status(404).json({
				...
				message: 'No existe un prisma.usuario con el ID proporcionado'
			});
		}

		if (medico.rol?.nombreRol !== 'medico') {
			return res.status(400).json({
				error: 'Tipo de prisma.usuario inválido',
				message: 'El prisma.usuario debe ser un médico para crear un perfil'
			});
		}
