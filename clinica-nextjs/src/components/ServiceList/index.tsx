...
export default function ServiceList() {
  ...
  const { isLoading, data = [], refetch } = useQuery<Servicio[]>({
    ...
    queryFn: async () => {
      ...
      return (res?.servicios ?? res?.items ?? res) as Servicio[]
    },
    staleTime: 60 * 1000,
  })
            ...
            <Table className="overflow-x-auto">
              ...
              <TableBody>
                {isLoading ? (
                  ...
                  data.map((svc) => {
                    const key = String(svc.estado).toLowerCase() as 'activo' | 'inactivo'
                      ...
                      <TableRow key={svc.id} className="hover:bg-muted/50">
                        <TableCell className="font-medium">
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              <div>{svc.nombre}</div>
                              <div className="text-xs text-muted-foreground">{svc.detalle || 'Sin descripción'}</div>
                            </div>
                          </div>
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell>{formatCRC(svc.precio)}</TableCell>
                        <TableCell className="text-sm">{formatRelative((svc as any).updatedAt)}</TableCell>
                        <TableCell>
                ...
                <CardContent className="p-4">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <h3 className="font-medium">{svc.nombre}</h3>
                        <p className="text-sm text-muted-foreground">{svc.detalle || 'Sin descripción'}</p>
                      </div>
                    </div>
                    <Badge className={cfg?.color}>{cfg?.label}</Badge>
                  </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <p className="text-sm font-medium">{formatCRC(svc.precio)}</p>
                    </div>
                    <div className="flex items-center justify-between">
                      <p className="text-sm text-muted-foreground">Actualizado</p>
                      <p className="text-sm font-medium">{formatRelative((svc as any).updatedAt)}</p>
                    </div>

                    <div className="flex gap-2 pt-2">
                      <Link href={`/admin/services/${svc.id}/edit`} className="flex-1">
