              ...
              <TableBody>
                {isLoading ? (
                  ...
                  data.map((service, index) => {
                    const key = (service.activo || '').toString().toLowerCase() as 'activo' | 'inactivo'
                    const cfg = statusConfig[key]
                    return (
                      <TableRow key={`service-${service.idServicio}-${index}`} className="hover:bg-muted/50">
                        <TableCell className="font-medium">
                          <div className="flex items-center gap-2">
                            <Package className="w-4 h-4 text-muted-foreground" />
                            <div>
                              <div>{service.nombreServicio}</div>
                        ...
                        <TableCell>
                          {service.precio == null ? '—' : formatCRC(service.precio)}
                        </TableCell>
                        <TableCell className="text-sm">
                          {formatRelative((service as any).updatedAt)}
                        </TableCell>
                        <TableCell>
                          <Link href={`/admin/services/${service.idServicio}/edit`}>
                          ...
                          <Button
                            ...
                            onClick={async () => {
                              ...
                              showNotification?.({
                                ...
                                message: `Se eliminó "${service.nombreServicio}"`,
                                type: "success",
                              })
                            }}
                          >
      ...
      <div className="lg:hidden space-y-4">
        {isLoading ? (
          ...
          data.map((service, index) => {
            ...
            const cfg = statusConfig[key]
            return (
              <Card key={`service-card-${service.idServicio}-${index}`} >
                <CardContent className="p-4">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <h3 className="font-medium">{service.nombreServicio}</h3>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2 pt-2">
                      <Link href={`/admin/services/${service.idServicio}/edit`} className="flex-1">
