...
export default async function ManageUser({
	...
	const stats = {
		...
		moderators: users.filter((u) => u.publicMetadata.role === "fisioterapeuta")
			.length,
		regular: users.filter((u) => u.publicMetadata.role === "paciente").length,
	};
								...
								<CardContent className="p-4 sm:p-5">
									<div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
										...
										<div className="grid grid-cols-1 gap-2 sm:w-[520px] sm:grid-cols-3">
											...
											<form action={setRole}>
												<input type="hidden" name="id" value={user.id} />
												<input type="hidden" name="role" value="fisioterapeuta" />
												<Button
													type="submit"
													size="sm"
