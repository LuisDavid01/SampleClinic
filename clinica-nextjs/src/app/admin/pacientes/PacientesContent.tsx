...
export default function PacientesContent() {
	...
	const loadPacientes = async (page = 1, search = "", retryCount = 0) => {
		try {
			setIsLoading(true);
			...
			console.log(`📋 Cargando pacientes - Página: ${page}, Búsqueda: "${search}", Móvil: ${isMobile}, Reintento: ${retryCount}`, {
				baseUrl: baseUrl,
				endpoint: apiEndpoints.getUsuarios(),
			...
			// Reintentar hasta 2 veces si es un error de conexión
			if (retryCount < 2 && (error?.message?.includes('conexión') || error?.message?.includes('Timeout') || error?.message?.includes('Failed to fetch'))) {
				console.log(`🔄 Reintentando carga de pacientes (intento ${retryCount + 1}/2)...`);
				setTimeout(() => {
					loadPacientes(page, search, retryCount + 1);
				}, 2000 * (retryCount + 1)); // Backoff exponencial: 2s, 4s
				return;
			}

			const errorMessage = error?.message || "No se pudo cargar la lista de pacientes";
					...
					{isLoading ? (
						...
						<div className="text-center py-8">
							<div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-6 max-w-md mx-auto">
								<Users className="w-12 h-12 mx-auto mb-4 text-red-500 opacity-50" />
								<p className="text-red-600 dark:text-red-400 font-semibold mb-2">Error al cargar pacientes</p>
								<p className="text-sm text-red-500 dark:text-red-400 mb-4">{error}</p>
								<Button
									onClick={() => loadPacientes(currentPage, searchTerm, 0)}
									variant="outline"
